...
<%
    ...
    if ("post".equalsIgnoreCase(request.getMethod())) {
        try {
            ...
            int cleanliness = Integer.parseInt(request.getParameter("cleanliness"));
            int servingSpeed = Integer.parseInt(request.getParameter("servingSpeed"));
            int staffBehaviour = Integer.parseInt(request.getParameter("staffBehaviour"));
            ...
            String checkSql = "SELECT COUNT(*) FROM guest_feedback WHERE GuestCode = ? AND submitted_at >= NOW() - INTERVAL 24 HOUR";
            PreparedStatement checkStmt = conn.prepareStatement(checkSql);
            checkStmt.setString(1, EmployeeCode);
            ResultSet rs = checkStmt.executeQuery();
            rs.next();
            int count = rs.getInt(1);

            if (count > 0) {
                msg = "You have already submitted feedback in the last 24 hours. Please try again later.";
            } else {
                // ✅ Step 2: Insert new feedback
                String sql = "INSERT INTO guest_feedback (GuestName, GuestCode, GuestDep, foodQuality, taste, Foodtaste, Fooddish, Foodmenu, Food, variety, cleanliness, servingSpeed, staffBehaviour, seating, availability, Foodreview, cafe, value, comments, submitted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
...
body { 
    ...
    padding:0; 
    min-height:100vh; 
    background:url('https://png.pngtree.com/thumb_back/fw800/background/20220828/pngtree-topview-of-indian-cuisine-dishes-and-food-south-frame-background-photo-image_28143440.jpg') no-repeat center center fixed; 
...
<div class="form-section employee-section">
    ...
    <div class="mb-3">
      <label class="form-label">Guest Company</label>
      <input type="text" name="EmployeeCompany" class="form-control" required>
    </div>
    <div class="mb-3">
      <label class="form-label">Guest Department</label>
      <select name="EmployeeDep" class="form-select" required>
        <option value="" disabled selected>Select department</option>
        <option value="It/Edp">It/Ce</option>
 ...
 <!-- FOOD RATINGS -->
    ...
    <div class="form-section cafe-section">
      ...
      <div class="mb-3">
        <label class="form-label">2. Recommend cafeteria to Other Guest?</label>
...
<script>
// Keep your star rating JavaScript
function setupStarRating(containerId, inputId) {
  ...
  function updateStars(rating, isHover=false) {
    stars.forEach((star,i)=>{ star.classList.remove("selected","hovered"); if(i<rating){ star.classList.add(isHover?"hovered":"selected"); }});
  }
}
